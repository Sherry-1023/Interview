![image](https://github.com/user-attachments/assets/3e372235-74d7-4ab6-a591-df96e7d64db5)<div align="center">
📖 Github
&emsp;&emsp; | &emsp;&emsp;
...
C++ 程序的编译和运行过程涉及多个步骤和组件。以下是简要的 C++ 编译运行全过程及其各个阶段的作用、输入和输出：

### 1. 编写源代码

- **输入**：程序员编写的 C++ 源代码文件（例如 `.cpp` 文件）。
- **输出**：源代码文件包含程序逻辑、数据定义等。

### 2. 预处理（Preprocessing）

- **作用**：处理预处理指令，如宏替换 `#define`、文件包含 `#include`、条件编译 `#if` 等，生成被处理后的源代码。
- **输入**：C++ 源代码文件。
- **输出**：经过预处理的源代码文件（通常为 `.i` 或 `.ii` 文件），移除了注释、替换了宏定义等。

### 3. 编译（Compilation）

- **作用**：将预处理后的源代码翻译成汇编语言。
- **输入**：经过预处理的源代码文件（`.i` 或 `.ii` 文件）。
- **输出**：汇编代码文件（通常为 `.s` 文件），包含与处理器架构相关的低级代码。

### 4. 汇编（Assembly）

- **作用**：将汇编代码翻译成目标机器码（二进制可执行代码）。
- **输入**：汇编代码文件（`.s` 文件）。
- **输出**：目标文件（`.o` 文件或 `.obj` 文件），包含机器指令和数据定义。

### 5. 链接（Linking）

- **作用**：将所有目标文件（`.o` 或 `.obj` 文件）及其所需的库文件链接在一起，生成最终的可执行文件。
- **输入**：目标文件、库文件（静态库 `.lib` 或动态库 `.dll/.so`）、系统提供的运行时库等。
- **输出**：可执行文件（`.exe` 或没有扩展名的可执行文件），包含完整的机器代码和数据，可被操作系统加载和执行。

### 6. 运行

- **作用**：操作系统加载可执行文件到内存中，并执行程序。
- **输入**：可执行文件。
- **输出**：程序运行的结果，可能是在终端显示、写入文件、生成输出文件等。
...
- **编写源代码**：
  - 输入：程序员编写的源代码文件。
  - 输出：源代码文件本身，包含程序逻辑和数据定义。

- **预处理**：
  - 输入：源代码文件。
  - 输出：经过预处理的源代码文件，移除了注释、替换了宏定义等。

- **编译**：
  - 输入：经过预处理的源代码文件。
  - 输出：汇编代码文件，包含与处理器架构相关的低级代码。
...
- **链接**：
  - 输入：目标文件、库文件、系统提供的运行时库等。
  - 输出：可执行文件，包含完整的机器代码和数据。

- **运行**：
  - 输入：可执行文件。
  - 输出：程序的运行结果，可以是在终端显示、生成输出文件等。
...
| 声明形式            | 含义                                      |
|---------------------|------------------------------------------|
| `const int &a` 或 `int const &a` | 引用，所引用的值不能修改                     |
| `const int *a` 或 `int const *a` | 指针，指向的值不能修改，但指针本身可以改变         |
| `int *const a`      | 指针本身不能改变，但指针指向的值可以修改         |
| `const int *const a`| 指针本身不能改变，指针指向的值也不能修改         | 


##### 1.2.3 为什么`*p=&a`不正确
`int *p=&a` 是对的，但是`*p=&a;`却不对?`int*p=&a;`这一句定义了一个变量`p` ，`p`是一个指向 `int`型 的指针，即`p`的数据类型是 `int*`，而`&a` 的数据类型也是 `int*`。这条语句可以分写为 `int *p;p=&a;`所以这一句没问题。
`*p-&a;`这一句是对`(*p)`进行赋值操作,`(*p)`的数据类型是 `int`型，而 `&a` 的数据类型是 `int*`，两者类型不同，无法进行赋值，所以语句是错的。
`*p=a` 和 `p=&a` 是正确的，`*p=a`的意思:将 `a`的值 赋给 `p`指针 指向的地址的值; `p=&a` 的意思是:将 `a`的地址 赋给 指针`p`;区别:`*p`是一个值;`p`是一个地址;两者完全不相同，`*`代表着,指向的地址的值，简单来说就是取值;`&`是取地址 符号，取的是 地址;`p`是指针，可以理解为所指向的值的地址，`*p`就是取 `p`指针 指向的地址的值， `&a` 就是取`a`的地址。
